...
IMPLEMENT_DYNAMIC(CAllControlsSheet, CMFCPropertySheet)
...
void CAllControlsSheet::AddControlPages()
{
	...
	m_calculationPage.m_psp.dwFlags &= ~PSP_HASHELP;
	m_resistorsPage.m_psp.dwFlags &= ~PSP_HASHELP;

	AddPage(&m_calculationPage);
	AddPage(&m_resistorsPage);
}
...
BOOL CAllControlsSheet::OnInitDialog()
{
	...
	pButton->SetWindowText(L"Close"); //change button text

	pButton = (CButton*)GetDlgItem(IDOK);	
	pButton->ShowWindow(SW_HIDE); //hide button
